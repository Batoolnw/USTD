...
def add_location(db_file: pathlib.Path, lat: float, lon: float, zoom: float, label: str):
    with sqlite3.connect(str(db_file)) as conn:
        ...
        try:
            ...
            cur.close()
    geo = '{lat}, {lon}'
    print('Added {label or geo} to database')
    ...
    try:
        while True:
            for loc in locations:
                ...
                combined = '{loc[1], loc[2]}'
                if new > 0:
                    print('Scraped {new} media from location {loc[4] or combined}')
                else:
                    print('No new media from location {loc[4] or combined}')
            if not repeat:
                break
...
def review(db_file: pathlib.Path, exe: str, label=None):
    ...
    for idx, (idnum, v) in enumerate(media):
        ...
        conn.commit()
        print('{len(media) - idx - 1} remaining')
...
def export(db_file: pathlib.Path, export_dir: pathlib.Path):
    ...
    for (media_path, timestamp, classification) in files:
        ...
        fname = '{date_str}-{classification[:200]}.mp4'
        dest = export_dir / fname
        if dest.exists():
            continue
        shutil.copy(str(media_file), str(dest))
    print('{len(files)} video(s) exported')
...
if __name__ == '__main__':
    ...
    if args.subparser_name == 'create':
        if args.database.exists():
            print("Database file '{args.database}' already exists.")
            sys.exit(1)
        create_database(args.database)
        sys.exit(0)

    if not args.database.exists():
        print("Database file '{args.database}' does not exist")
        sys.exit(1)
